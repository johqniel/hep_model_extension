...
module mod_agent_world
  ...
  implicit none
   ...
   type :: Agent
      ...
      integer :: tsb_in_ticks = 0                            ! time since birth in ticks (for fertility)
    ...
    type :: world_container
        ...
        contains 
            ...
            procedure, public :: agent_born ! we want to change this to private once old program is removed.
            procedure, public :: spawn_agent_hash    ! same
            procedure, public :: add_agent_to_array_hash
...
contains
  ...
  ! ==========================================================================
    ...
    function agent_born(self, mother, father) result(agent_spawned)
        ...
        call self%add_agent_to_array_hash(temp_agent, population, child_ptr)
      ...
      ! ################## Gene propagation ####################################
        ...
        function spawn_agent_hash(self,population) result(agent_spawned)
            ...
            ! Initialize tsb_in_ticks (time since birth for fertility)
            ! For initial females, set to > 2 years to bypass fertility lockout
            if (agent_spawned%gender == 'F') then
                agent_spawned%tsb_in_ticks = int(2.5d0 / self%config%dt)  ! Set to 2.5 years
            else
                agent_spawned%tsb_in_ticks = 0
            end if
  ...
  function get_agent(id, world_ptr) result(agent_ptr)
    ...
    function get_ith_agent_from_cell(world_ptr,i ,gx,gy) result(agent_ptr)
        implicit none
        type(world_container),target, intent(inout) :: world_ptr
        ...
        agent_ptr => get_agent(agents_id, world_ptr)


    end function

    function get_first_male_from_cell(world_ptr, gx, gy) result(agent_ptr)
        implicit none
        type(world_container),target, intent(inout) :: world_ptr
        ...
        integer :: agents_id
        type(Grid), pointer :: grid
        type(grid_cell), pointer :: grid_cell
        ...
        grid_cell => grid%cell(gx,gy)

        number_of_agents = grid_cell%number_of_agents
        ...
        if (number_of_agents < 1) then
            print*, "Warning: Can not find male in empty cell. "
            world_ptr%counter%get_male_fails = world_ptr%counter%get_male_fails + 1
            return
        endif
        ...
        do i = 1, number_of_agents
            ...
            ! AGENT_PTR -> ITH AGENT
            agent_ptr => get_ith_agent_from_cell(world_ptr,i,gx,gy)

            if (agent_ptr%gender == 'M') then
                exit
            endif
        ...
        if (.not. associated(agent_ptr)) then
            print*, "Warning: i in get_male is out of bounds, i = ", i
            print*, " there is no male found in the cell: ", gx, ", " ,gy
            world_ptr%counter%get_male_fails = world_ptr%counter%get_male_fails + 1
        endif


        ! if male was found it is returned, otherwise null() is returned 

        ! how to use: 

        !agent_ptr => get_first_male_from_cell

        !if (.not. associated(agent_ptr)) 
        !    print("didnt find male")
        !    return
        !end if

    end function get_first_male_from_cell
