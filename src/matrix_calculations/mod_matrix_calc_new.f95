          ...
          subroutine setup_update_human(jp)
            ...
            !        !$OMP DO
                ...
                logical function is_agent_alive(i,jp)
                    ...
                    is_agent_alive_temp = .true.

                    if (x(i,jp) <= -900 .or. y(i,jp) <= -900) then
                    ...
                    is_agent_alive = is_agent_alive_temp

                end function is_agent_alive

                logical function agent_in_research_area(i,jp)
                    implicit none
                    integer, intent(in) :: i, jp
                    logical :: agent_in_research_area_temp

                    agent_in_research_area_temp = .true.

                    if ((x(i,jp)<lon_min_out) .OR. (x(i,jp)>lon_max_out)) then 
                       agent_in_research_area_temp = .false.
                    endif

                    if ((y(i,jp)<lat_min_out) .OR. (y(i,jp)>lat_max_out)) then
                        agent_in_research_area_temp = .false.
                    endif

                end function agent_in_research_area

                logical function agent_above_water(i,jp)
                    implicit none 
                    integer, intent(in) :: i, jp
                    logical :: agent_above_water_temp

                    agent_above_water_temp = .false.

                    if(hep(gx, gy, jp, t_hep) <= 0. )    then
                        agent_above_water_temp = .true.
                    endif

                    agent_above_water = agent_above_water_temp
                end function agent_above_water

                subroutine calculate_gradient(i,jp) ! In this function there is some kind of coordinate transformation I think we should isolate
