...
!----------------------------------------------------------------------------------------------
    ...
    subroutine birth_death_euler_new(grid,agent_matrix,sigma_u,rho_adj, hep, lat_in, lon_in, r_B, d_B, N_max,dt_yr,imus,population,hum_t)
      implicit none 
      type(spatial_grid), intent(in) :: grid                               ! The grid object on which the agents are moving
      type(pointer_node), allocatable, intent(in) :: agent_matrix(:)       ! array of pointer_nodes that contain agents of one population
      ...
      integer                                :: imus      ! which time step

      integer                                :: population ! the population that is beeing simulated
      integer, dimension(:), intent(inout)   :: hum_t      ! array that contains the number of humans in each population
    ...
    !
        ...
        integer :: pop_size_old
                            ...
                            print*, "This function was disabled (cannot kill  and born agents anymore). - birth_death_euler1"
