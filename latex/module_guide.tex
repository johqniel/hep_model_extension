...
In \texttt{python/utils.py}, add your module to the central \texttt{MODULE\_REGISTRY} list (specify its \texttt{id}, \texttt{name}, \texttt{group}, \texttt{author}, and source \texttt{file}). The ID must match the Fortran constant:
\begin{lstlisting}[language=Python]
MODULE_REGISTRY = [
    # ... existing modules ...
    {"id": 17, "name": "My New Behavior", "group": "ReviewedModules", "author": "Your Name", "file": "mod_my_behavior.f95"},
]
\end{lstlisting}
The rest of the framework (such as \texttt{spawn\_editor.py}, \texttt{simulation.py}, \texttt{headless\_simulation.py}, and \texttt{export\_simulation.py}) imports this list and builds its internal mappings automatically.
...
\begin{itemize}
    ...
    \item \textbf{MinPts}: The number of samples in a neighborhood for a point to be considered as a core point.
\end{itemize}

\subsection{Convex Hull Fill}
To improve visual consistency, a \textbf{Convex Hull Post-Processing} step is applied to clustered grid cells. This fills internal holes and noise gaps within identified clusters, preventing cross-cluster contamination while maintaining structural integrity.

% =====================================================================
\section{Visualization and Rendering}
% =====================================================================

The Python visualization engine (\texttt{simulation.py}) handles the translation of Fortran grid data into interactive maps.

\subsection{Rendering Architecture}
\begin{itemize}
    \item \textbf{Bifurcated Loops}: 2D and 3D visual rendering are structurally independent. The \texttt{update\_visualization} method utilizes different mathematical paths for drawing flat grids (\texttt{ImageItem}) versus interpolating spherical terrains (\texttt{GLMeshItem}).
    \item \textbf{RGBA Matrices}: The backend simulation pipeline expects arrays formatted exactly as \textbf{4-channel RGBA} (\texttt{R, G, B, Alpha}). Boundary edges and UI markers are "burned" directly into these matrices inside Python to ensure rendering certainty.
\end{itemize}
